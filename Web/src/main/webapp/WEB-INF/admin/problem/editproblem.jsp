...
<script>
    ...
    $.getJSON("/api/problem?pid="+pid, function (res) {
        ...
        $("#problemDesc").html(res.desc);
        $("#problemTimeLimit").text(res.timelimit);
        $("#problemMemoryLimit").text(res.memorylimit);
        $("#problemHint").html(res.hint);
        $("#problemType option:nth-of-type("+(res.type+1)+")").addClass("selected")
        $("#problemSpecial option:nth-of-type("+(res.special?2:1)+")").addClass("selected")
    })

    $("#problemTitle").on("click",function () {
        let a = prompt("输入新标题",$("#problemTitle").text());
        if (a != null) {
            $("#problemTitle").text(a);
        }
    })
